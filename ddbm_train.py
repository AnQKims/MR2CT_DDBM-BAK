...
def main(args):
    ...
    if dist.get_rank() == 0:
        ...
        wandb.init(project="bridge", 
                   ...
                   name=name, 
                   config=vars(args), 
                   mode="disabled") # 禁用wandb，不再联网保存云端日志
        
        wandb.init(project="bridge", 
                   group=args.exp,
                   name=name, 
                   config=vars(args), 
                   mode="offline") #  本地保存 wandb 日志 DDBM/wandb/
    ...
    # 自动恢复训练，只要 workdir 下有 checkpoint，就会接着跑。
...
"""
把“所有可调训练参数”暴露为命令行参数
    python ddbm_train.py \
    --data_dir ./data \
    --dataset edges2handbags \
    --image_size 256 \
    --lr 1e-4 \
    --resume_checkpoint workdir/model_050000.pt
...
def create_argparser():
    ...
    defaults.update(model_and_diffusion_defaults())# 它返回：UNet 结构参数、diffusion 参数、image_size、num_channels、sigma schedule、bridge / VP / VE 相关参数
